...
class Contract {
   ...
   * @param {string} contract.payment_destination - Zcash address to send funds
   ...
   * @param {buffer} signature - The contract signature to verify
  ...
  static from(data) {
    if (data instanceof Contract) {
      return Contract.fromObject(data.toObject());
