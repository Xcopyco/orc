...
class Node extends KademliaNode {
  ...
  identifyService(uri, callback) {
    const req = https.request(merge(url.parse(uri), {
      agent: this.onion.secureAgent,
      method: 'GET'
    }), (res) => {
      let body = '';
  ...
  subscribeShardDescriptor(descriptorCodes, callback) {
    ...
    this.quasarSubscribe(descriptorCodes, ([descriptor, contact]) => {
      ...
      if (descriptor.isValid()) {
        descriptorStream.push([descriptor, contact]);
      }
    });
   ...
   * @param {number} bytesAvailable - Total bytes capacity available
  ...
  publishCapacityAnnouncement(descriptorCode, bytesAvailable, callback) {
    ...
    this.claims = bytesAvailable > 0 ? this.claims : [];
