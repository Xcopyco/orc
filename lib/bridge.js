    ...
    const prepareShards = (file, rsparams, object, callback) => {
      ...
      const prepareContracts = () => {
        async.eachSeries(shards, (shard, next) => {
          ...
          this.objects.save(object.hash, object, () => callback(null, object));
        });
      }
    ...
    const uploadShards = (shards, object, callback) => {
      async.eachLimit(shards, 3, (shard, next) => {
        async.retry({ times: 5 }, (done) => {
          this.capacityCache.find({})(capacities => {
            let proposal = shard._proposal;
            let target = null;
            ...
            for (let i = 0; i < capacities.length; i++) {
              ...
              target = this.node.router.getContactByNodeId(capacities[i]._key);
              ...
              uploadStream.on('response', (res) => {
                ...
                res.on('end', () => {
                  if (res.statusCode !== 200) {
                    this.node.logger.warn(
                      `failed to upload shard, reason: ${err.message}`
                    );
                    return done(new Error(body));
                  }

                  shard._contract = completedContract;
                  done();
                });
              });
              fs.createReadStream(shard.path).pipe(uploadStream);
            });
          }, next);
        });
      }, (err) => {
        if (err) {
          ...
          this.object.set(object._key, object);
          async.eachLimit(
            object.shards,
            3,
            (shard, next) => fs.unlink(shard.path, next),
            (err) => {
              if (err) {
                return callback(err);
              }

              fs.unlink(path.dirname(filepath), callback);
            }
          );
        }
      });
